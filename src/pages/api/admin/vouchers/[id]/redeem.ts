...
import { redeemAdminVoucher } from "@/server/admin/admin-voucher-actions.service";
...
export async function POST({ request, params, locals }: APIContext) {
  ...
  try {
    const result = await redeemAdminVoucher(locals.admin, params.id ?? "");
    ...
    if (!result.success) {
      ...
      return jsonResponse(
        { success: false, message: "Tego vouchera nie można oznaczyć jako wykorzystany." },
        409,
      );
    }
    ...
    console.error("Admin voucher redeem failed:", { voucherId: params.id, error });
    return jsonResponse({ success: false, message: "Nie udało się zaktualizować vouchera." }, 500);
  }
}
